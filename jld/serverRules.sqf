...
서버가 업데이트 되었습니다. 업데이트 내용은 아래와 같습니다.

1.일부 장비의 건설비용이 수정되었습니다.
-무인장비(고정무인포탑 제외)의 인력과 연료 소모량이 대폭 증가하였습니다.
-콘크리트 경사로의 건설비용이 100으로 증가하였습니다.

2.승무원 할당 스크립트가 적용되었습니다.
-이전에 전차장 좌석에서 운전수를 추가하던 스크립트가 아래와 같이 변경됩니다.
-이제 전차, 차륜형 APC, 모든 항공기의 운전석에서 부조종사와 포탑 사수를 배치할 수 있습니다.
-승무원은 운전중인 유저가 좌석을 옮기거나, 내리거나, 사망하는경우 사라집니다.
-인질을 전차에 탑승시키고 운전석에 탑승하는경우 버그로 인해 인질이 사라지는 현상을 수정하였습니다.

3.전술통신망 스크립트가 최적화되었습니다.
-이제 전술통신망에 가입한 사람이 맵상에 주황색으로 표시됩니다.
-차량의 경우 탑승인원중 한명이라도 전술통신망에 가입되어 있다면 주황색으로 표시됩니다.

4.서버 접속시 나오는 안내문구가 간소화되었으며, 디스코드에 주력하도록 변경되었습니다.
-그래봤자 바로 엔터로 넘기시는건 다 알고있습니다.

5.서버 최적화 스크립트의 수치를 조절하여 파괴된 차량잔해와 건물잔해를 빠르게 삭제하도록 조절하였습니다.

6.폭발물 설치와 특수임무 실행시 채팅창을 통해 전파하도록 변경되었습니다.

7.무전기 가방을 통해 적군을 수색하는 스크립트의 수치를 일부 조정하여 적 보병의 신호를 찾기 쉽도록 조절하였습니다.

8.보급차량 주변에서 1분간 기다리면 자동으로 재보급되는 스크립트를 해제하였습니다.(PPAP스크립트와 충돌)

9.윈도우키 스크립트가 수정되었습니다.
-추가설정메뉴가 윈도우키 메뉴로 이동되었으며, 메뉴를 나누기 위한 분리대를 추가하였습니다.
-윈도우키를 누른 상태에서 화면 좌측하단에 HP상태뿐만 아니라 점령보너스 획득상태, 이어플러그 착용상태, 전술통신망 연결 상태를 확인할 수 있도록 수정하였습니다.
-전술 수신호를 사용할 때 주변 30m의 플레이어에게 화면 중앙하단에 적절한 대사가 나오도록 하였으며 항공기 유도 수신호가 추가되었습니다.

10.알티스의 기후가 일부 조절되었습니다.
-알티스가 조금더 화창해졌으며 안개의 양을 정규분포의 역을 따르도록 수정하였습니다. 
-쉽게말해서, 맑거나 흐리거나! 어정쩡한 안개는 줄어들었습니다!
-날씨변화가 더 천천히 이루어지도록 수정하였습니다.

11.고정무기 적재 스크립트가 최적화되었습니다.
-더이상 고정무기 하역을 통해 차량에 부착된 화물이 하역되어 igiLoad 스크립트와 충돌을 일으키지 않습니다.

12.[중요]조종사 플레이와 무인장비 조작을 하기 위해 특정 쇼케이스를 완수해야 합니다.
-완수해야하는 쇼케이스는 무인기/공격헬기/고정익/회전익 쇼케이스 4종입니다.
-위 4종의 쇼케이스중 하나라도 완수하지 않은 경우에는 조종사와 무인기 조작이 불가능합니다.

13.[중요]무인기 관련 규정과 스크립트가 수정되었습니다.
-무인기 조작을 위해서도 12번 수정내용의 4개 쇼케이스를 완수해야 합니다.
-더이상 무인기의 운전석과 포탑을 수동조작할 수 없도록 변경되었습니다.
-[특수]분대의 규정이 아래와 같이 수정되었습니다. (무인기는 분대당 한대씩만 사용가능합니다. 편법적으로 다수의 특수분대를 운용하는 경우 킥,밴될 수 있습니다.)
