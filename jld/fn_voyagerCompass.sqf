...
fnc_showSettings =
{
  ...
  nc_dialog_controls = [
    ...
    [1114,"nc_background_info_soon",[0,"more features soon",[0.314375 * safezoneW + safezoneX,0.874 * safezoneH + safezoneY,0.0825 * safezoneW,0.022 * safezoneH],[1,1,1,0.3],[0,0,0,0],[-1,-1,-1,-1],"INFO. Future updates coming. Stay tuned","RscText"],[]],
  ...
  while {true} do {
    ...
    /// create double array ([marker], [idc,marker])
    {
      if !(_x in currentMarkers) then {
        ...
        //};
        systemChat format ["지도에 마크가 추가되었습니다.", _x];
      };
