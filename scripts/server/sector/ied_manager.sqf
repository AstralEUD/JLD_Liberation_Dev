...
_activation_radius_infantry = 15;
_activation_radius_vehicles = 7;

_spread = 15;
_infantry_trigger = 2 + (floor (random 2));
_ultra_strong = false;
if ( random 100 < 20 ) then {
	_ultra_strong = true;
};
_vehicle_trigger = 1;
_ied_type = [ "IEDLandBig_F","IEDLandSmall_F","IEDUrbanBig_F","IEDUrbanSmall_F","APERSBoundingMine","APERSTripMine" ] call BIS_fnc_selectRandom;
_ied_obj = objNull;
...
if ( !(isnull _roadobj) ) then {
	...
	_ied_obj setPos [getPos _ied_obj select 0,getPos _ied_obj select 1, -0.05];
