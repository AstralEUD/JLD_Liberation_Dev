	...
	while { true } do {
...

		{
			if ( ( side group _x == GRLIB_side_friendly ) && ( !isPlayer _x ) ) then {
				if ( ( _x distance lhd > 250 ) && ( _x distance chimeraofficer > 250 ) && ( _x distance ( getmarkerpos GRLIB_respawn_marker) > 100 ) && ( alive _x ) ) then {
					_unit = _x;
					{
		...
		{
			if ( ( _x distance lhd > 250 ) && ( _x distance chimeraofficer > 250 ) && ( alive _x ) ) then {
				_unit = _x;
				{
					if ( ( _x select 0 ) == typeof _unit ) then {
						_new_manpower_used = _new_manpower_used + (_x select 1);
