...
do {
	...
	if (GRLIB_removefobboxes)
	then {
		GRLIB_removefobboxes = false;
		if (count _nearfobbox > 0)
		then {
	...
	if (count _nearfobbox != 0 && alive player && vehicle player == player && !(surfaceIsWater getpos player) && (((player distance lhd) > 1000) || (player distance huronspawn) > 1000) && (([player, 3]call F_fetchPermission) || (player == ([]call F_getCommander) || []call F_isAdmin)))
	then {
		if (_idact_buildfob == -1)
		then {
	...
	if ((leader group player == player) && (count units group player > 1) && alive player && vehicle player == player)
	then {
		if (_idact_squad == -1)
		then {
	...
	if ((player == ([]call F_getCommander) || []call F_isAdmin) && alive player && vehicle player == player && GRLIB_permissions_param)
	then {
		if (_idact_commander == -1)
		then {
	...
	if (_fobdistance < _distredeploy && alive player && vehicle player == player && (player == ([]call F_getCommander) || []call F_isAdmin))
	then {
		if (_idact_repackage == -1)
		then {
