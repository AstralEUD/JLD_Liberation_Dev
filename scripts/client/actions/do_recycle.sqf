...
_objectinfo = ([(light_vehicles + heavy_vehicles + air_vehicles + static_vehicles + support_vehicles + buildings), {
			typeof _vehtorecycle == _x select 0
		}
...
while {
	dialog && (alive player) && dorecycle == 0
}
do {
	sleep 0.1;
};
...
if (dorecycle == 1 && !(isnull _vehtorecycle) && alive _vehtorecycle)
	then {
		[[_vehtorecycle, round((_objectinfo select 2) * GRLIB_recycling_percentage)], "recycle_remote_call"]call BIS_fnc_MP;

		//_vehiclename = getText(configFile >> "cfgVehicles" >> _vehtorecycle >> "displayName");
		_builder = name player;
		format["%2님이 %1을 재활용하였습니다.", getText(_cfg >> (_objectinfo select 0) >> "displayName"), _builder]remoteExec["systemChat"];
	};
