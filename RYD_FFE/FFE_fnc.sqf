...
RYD_CFF_TGT = { //_tgt = [RydFFE_KnEnemies] call RYD_CFF_TGT;
	...
	_nothing = 0;{
		...
		if not(isNull _potential)then {
			if (alive _potential)
				then {
					if not(_taken)then {
						if (((getposATL _potL)select 2) < 20)
							then {
								if ((abs(speed _potL)) < 50)
									then {
										if ((count(weapons(leader _potential))) > 0)
											then {
	...
	foreach _enemies;{
		...
		if not((vehicle _CL) == _CL)then {
			_veh = vehicle _CL;
			if ((toLower(typeOf _veh))in RydFFE_AllArty)
				then {
					_artFactor = 10
				}
			else {
		...
		_nearCiv = false;{
			if (_x isKindOf "civilian")
				exitWith {
					_nearCiv = true
				};
			if (((side _x)getFriend(side _CL)) >= 0.6)
				then {
					_vh = vehicle _x;
					_crowdFactor = _crowdFactor + 0.2;
					if not(_x == _vh)then {
						_crowdFactor = _crowdFactor + 0.2;
						if ((toLower(typeOf _vh))in RydFFE_AllArty)
							then {
								_crowdFactor = _crowdFactor + 0.2
							}
					}
	...
	_ValMax = 0;{
		...
		if ((_ValMax < _trgValS)and(random 100 < 85))
			then {
				_ValMax = _trgValS;
				_target = _x
			};
...
RYD_ArtyMission = { //_bArr = [_tgtPos,RydFFE_ArtG,"SADARM",6,leaderHQ] call RYD_ArtyMission;
...
default {
		if ((count RydFFE_Other) > 0)
			then {
				_arr = [];{
					if (_tp in(_x select 0))
						exitWith {
							_arr = _x select 1
						}
				}
...
RYD_CFF_FFE = { //[_battery,_tgt,_batlead,"SADARM",RydFFE_Friends,RydFFE_Debug] spawn RYD_CFF_FFE
	...
	if ((_isTaken)and(RydFFE_Monogamy))
		exitWith { {
				if not(isNull _x)then {
					_x setVariable["RydFFE_BatteryBusy", false]
				}
	...
	_phaseF = [1, 2];
	if ((RydFFE_OnePhase)or((count RydFFE_FO) == 0)and not(RydFFE_2PhWithoutFO))
		then {
			_phaseF = [1]
		};
	...
	_amount2 = _amount - _amount1;{
		...
		if ((abs(speed _target)) > 50)
			exitWith {
				_waitFor = false
			};
		if ((_assumedPos select 2) > 20)
			exitWith {
				_waitFor = false
			};

		if ((_assumedPos distance[0, 0, 0]) == 0)
			exitWith {
				_waitFor = false
			};
		...
		if not((str _againcheck) == (str _target))then {
			_againF = 1
		};
		...
		if (isNil("RydFFE_Safe"))
			then {
				_safebase = 100
			}
		else {
			_safebase = RydFFE_Safe
		};
		...
		if not(_onRoad)then { {
				if (([(_impactpos select 0) + _Xhd, (_impactpos select 1) + _Yhd]distance(vehicle(leader _x))) < _safe)
					exitwith {
						_Xhd = _Xhd / 2;
						_Yhd = _Yhd / 2
					}
...
if ((_impactpos distance(vehicle(leader _x))) < _safe)
	exitwith {
		_safeCheck = false;
...
if not(isNil("RydFFE_FOAccGain"))then {
	_spotterF = RydFFE_FOAccGain + (random 0.2)
};
if (((count _phaseF) == 2)and(_x == 1)or((count _phaseF) == 1))
	then {
		_spotterF = 1
	};
...
[_battery, _distance, _eta, _ammoG, _batlead, _target, _markers]spawn {
	...
	waitUntil {
		...
		if (_battery1 getVariable["RydFFE_Break", false])
			then {
				_alive = false
			};{
			if not(isNull _x)then { {
					if ((vehicle _x)getVariable["RydFFE_ShotFired", false])
						exitWith {
							_shot = true
						}
				}
	...
	while {
		(not(_rEta < 5)and not(_TOF > 200)and(_alive))
	}
	do {
		if (({
				not(isNull _x)
...
if not(_alive)then {
	(_battery select 0)setVariable["RydFFE_Break", true]
};
...
waituntil {
	...
	if (({
		...
		if not(isNull _x)then { {
				if not((vehicle _x)getVariable["RydFFE_GunFree", true])exitWith {
					_available = false
				}
			}
			foreach(units _x)
...
if (((count _phaseF) == 2)and(_x == 1))
	then {
		...
		waitUntil {
			...
			if not(isNull _battery1)then {
				_splash = _battery1 getVariable["RydFFE_SPLASH", false]
			};
		...
		if not(isNull _battery1)then {
			_battery1 setVariable["RydFFE_SPLASH", false]
		};
...
RYD_CFF = { //[RydFFE_ArtG,RydFFE_KnEnemies,(RydFFE_EnHArmor + RydFFE_EnMArmor + RydFFE_EnLArmor),RydFFE_Friends,RydFFE_Debug] call RYD_CFF;
...
RYD_CFF_Fire = {
	...
	do {
		...
		if (not(_rest0 > _rest)and(_rest > 0))
			exitWith {
				_bad = true
			}
	};
...
if not(isNull _x)then {
	...
	if ((_vh getVariable["RydFFE_ShotsToFire", 0]) > 0)
		then {
			...
			if (_ammoC in _mags)
				then {
					...
					[_vh, _pos, _ammoC]spawn {
						...
						if (_pos inRangeOfArtillery[[_vh], _ammo])
							then {
								if (_ammo in(getArtilleryAmmo[_vh]))
									then {
										...
										if (_pos inRangeOfArtillery[[_vh], _ammo] && (side _vh == east))
											then {
												if (_ammo in(getArtilleryAmmo[_vh]))
													then {
														if (((toLower(typeOf _vh))in["uss_iowa_turret_c", "uss_iowa_turret_b", "uss_iowa_turret_a"])or {
															...
															//saky
															_radsAlive = [];
															_eta = _vh getArtilleryETA[_pos, _ammo];
															_posM1 = getPosATL _vh;
															_posM1 set[2, 0];
															_rads = nearestObjects[_posM1, ["B_Radar_System_01_F", "I_LT_01_scout_F"], 10000];{
																if (alive _x && (speed _x < 0.1))
																	then {
																		_radsAlive pushBack _x;
																	};
															}
															foreach _rads;
															if (count _radsAlive > 0)
																then {
																	_radPos = getPosATL(_radsAlive select 0);
																	_radPos set[2, 0];
																	_radacc = 0;
																	if ((_radsAlive select 0)isKindOf "Tank")
																		then {
																			_radacc = 0.0000003 * (_posM1 distance _radPos) ^ 3;
																		}
																	else {
																		_radacc = 0.00000002 * (_posM1 distance _radPos) ^ 3;
																	};
																	if (_radacc < 10000)
																		then {
																			_markers = [];
																			//[[west, "Base"], format["적 포격이 감지되었습니다!!", _radacc]]remoteExec["sideChat"];

																			_text = getText(configFile >> "CfgVehicles" >> (typeOf(vehicle _vh)) >> "displayName");
																			_i = "markBat" + str(_vh);
																			_i = createMarker[_i, _posM1 getPos[random _radacc, random 360]];
																			_i setMarkerColor "ColorRed";
																			_i setMarkerShape "ICON";
																			_i setMarkerType "o_art";
																			_i setMarkerSize[2, 2];
																			_i setMarkerText(format["ETA:%1s", _eta]);
																			_markers pushBack _i;

																			_i = "markBat" + str(_rads select 0);
																			_i = createMarker[_i, _radPos];
																			_i setMarkerColor "ColorGreen";
																			_i setMarkerShape "ICON";
																			_i setMarkerType "b_recon";
																			_i setMarkerSize[1, 1];
																			_i setMarkerText(format["DETECTING RADAR"]);
																			_markers pushBack _i;
	...
	_inRange = [];{
		...
		if not(isNil "_shell")then {
			if not(isNull _x)then {
				...
				if ((_pos1 distance _pos2) < _radius)
					then {
